...
public class PendingGameState : IGameState
{
    ...
    public void HandleResign(Game game, int playerId) => throw new InvalidOperationException("game has not started yet");

    public void HandleTimeout(Game game, int playerId) => throw new InvalidOperationException("game has not started yet");

    public void HandleDisconnect(Game game, int playerId) => game.SetState(new FinishedGameState());
}